<?php

namespace App\Http\Controllers\Backend;

use App\Http\Controllers\Controller;
use App\Services\CertificationService;
use App\Models\Language;
use App\Http\Requests\Backend\Certification\StoreCertificationRequest;
use App\Http\Requests\Backend\Certification\UpdateCertificationRequest;
use Illuminate\Http\Request;
use Yajra\DataTables\Facades\DataTables;

class CertificationController extends Controller
{
    protected $service;

    public function __construct(CertificationService $service)
    {
        $this->service = $service;
    }

    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = $this->service->getAll();

            return DataTables::of($data)
                ->addIndexColumn()
                ->addColumn('name', function ($row) {
                    $name = $row->getTranslated('name', '—');
                    $icon = $row->icon_class ? '<i class="' . e($row->icon_class) . ' me-2"></i>' : '';
                    return $icon . e($name);
                })
                ->addColumn('description', function ($row) {
                    $desc = $row->getTranslated('description', '—');
                    return \Illuminate\Support\Str::limit(strip_tags($desc), 60);
                })
                ->addColumn('status', function ($row) {
                    if ($row->is_active) {
                        return '<span class="badge bg-light-success text-success">Active</span>';
                    }
                    return '<span class="badge bg-light-danger text-danger">Inactive</span>';
                })
                ->addColumn('action', function ($row) {
                    $editUrl = route('admin.certifications.edit', $row->id_certification);
                    $btn = '<a href="' . $editUrl . '" class="btn btn-icon btn-light-primary btn-sm" title="Edit"><i class="ti ti-edit"></i></a>';
                    $btn .= ' <a href="javascript:void(0)" data-id="' . $row->id_certification . '" class="btn btn-icon btn-light-danger btn-sm btn-delete" title="Delete"><i class="ti ti-trash"></i></a>';
                    return $btn;
                })
                ->rawColumns(['name', 'status', 'action'])
                ->make(true);
        }

        return view('backend.certifications.index');
    }

    public function create()
    {
        $sortedLanguages = Language::where('is_active', true)->orderBy('is_default', 'desc')->get();
        return view('backend.certifications.create', compact('sortedLanguages'));
    }

    public function store(StoreCertificationRequest $request)
    {
        try {
            $data = $request->validated();
            $this->service->create($data);
            return redirect()->route('admin.certifications.index')->with('success', 'Certification created successfully.');
        } catch (\Exception $e) {
            return back()->with('error', 'Failed to create certification: ' . $e->getMessage())->withInput();
        }
    }

    public function edit($id)
    {
        try {
            $certification = $this->service->findById($id);
        } catch (\Exception $e) {
            return redirect()->route('admin.certifications.index')->with('error', 'Certification not found.');
        }

        $sortedLanguages = Language::where('is_active', true)->orderBy('is_default', 'desc')->get();
        return view('backend.certifications.edit', compact('certification', 'sortedLanguages'));
    }

    public function update(UpdateCertificationRequest $request, $id)
    {
        try {
            $data = $request->validated();
            $data['is_active'] = (int) $request->input('is_active', 0);
            $this->service->update($id, $data);
            return redirect()->route('admin.certifications.index')->with('success', 'Certification updated successfully.');
        } catch (\Exception $e) {
            return back()->with('error', 'Failed to update certification: ' . $e->getMessage())->withInput();
        }
    }

    public function destroy($id)
    {
        try {
            $deleted = $this->service->delete($id);
            if ($deleted) {
                return response()->json(['success' => true, 'message' => 'Certification deleted successfully.']);
            }
            return response()->json(['success' => false, 'message' => 'Certification not found.'], 404);
        } catch (\Exception $e) {
            return response()->json(['success' => false, 'message' => 'Failed to delete certification: ' . $e->getMessage()], 500);
        }
    }
}
