...
class UpdateBlogPostRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $postId = $this->route('blog_post')->id_blog_post ?? null;
        ...
        foreach ($languages as $lang) {
            $code = $lang->code;
            $translationId = \App\Models\BlogPostTranslation::where('id_blog_post', $postId)
                ->where('locale', $code)
                ->value('id_blog_post_translation');

            $uniqueRule = 'unique:blog_post_translations,slug';
            if ($translationId) {
                $uniqueRule .= ',' . $translationId . ',id_blog_post_translation';
            }
