...
@section('content')
    ...
    <form action="{{ route('admin.blog-posts.update', $post->id_blog_post) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="tab-content" id="languageTabContent">
                            @foreach($sortedLanguages as $language)
                                @php
                                    $translation = $post->translate($language->code);
                                @endphp
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" id="excerpt_{{ $language->code }}"
                                            ...
                                            rows="3">{{ old('translations.' . $language->code . '.excerpt', $translation->excerpt ?? '') }}</textarea>
                                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_blog_category" id="id_blog_category"
                                class="form-control @error('id_blog_category') is-invalid @enderror" required>
                                <option value="">Select Category</option>
                                @foreach($categories as $id => $name)
                                    <option value="{{ $id }}" {{ old('id_blog_category', $post->id_blog_category) == $id ? 'selected' : '' }}>{{ $name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3" id="published_at_container"
                            style="display: {{ old('status', $post->status) == 'published' ? 'block' : 'none' }};">
                            ...
                            <small class="text-muted">Leave empty to use the current time if publishing.</small>
                            @error('published_at')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('reading_time') is-invalid @enderror"
                                id="reading_time" name="reading_time" value="{{ old('reading_time', $post->reading_time) }}"
                                min="0">
                            @error('reading_time')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            @if($post->image_featured)
                                <div class="mb-2">
                                    <img src="{{ asset($post->image_featured) }}" alt="Featured Image" class="img-thumbnail"
                                        style="max-height: 150px;">
                                </div>
                            @endif
                            <input type="file" class="form-control @error('image_featured') is-invalid @enderror"
                                id="image_featured" name="image_featured" accept="image/*">
