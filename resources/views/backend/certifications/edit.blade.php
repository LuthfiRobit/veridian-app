...
@section('content')
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Edit Certification</h5>
                    </div>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item">Company</li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.certifications.index') }}">Certifications</a>
                        </li>
    ...
    <form action="{{ route('admin.certifications.update', $certification->id_certification) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="tab-content">
                            @foreach($sortedLanguages as $language)
                                @php $t = $certification->translate($language->code); @endphp
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                    id="lang-{{ $language->code }}" role="tabpanel">
                                    <div class="mb-3">
                                        <label class="form-label">Name ({{ $language->code }}) @if($loop->first)<span
                                        class="text-danger">*</span>@endif</label>
                                        <input type="text"
                                            class="form-control @error('translations.' . $language->code . '.name') is-invalid @enderror"
                                            name="translations[{{ $language->code }}][name]"
                                            value="{{ old('translations.' . $language->code . '.name', $t?->name) }}"
                                            @if($loop->first) required @endif>
                                        @error('translations.' . $language->code . '.name')<div class="invalid-feedback">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text"
                                            ...
                                            value="{{ old('translations.' . $language->code . '.description', $t?->description) }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="is_active">
                                <option value="1" {{ old('is_active', $certification->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $certification->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            new TomSelect("#icon_class", {
                maxOptions: null,
                options: iconOptions,
                items: {!! json_encode([old('icon_class', $certification->icon_class)]) !!},
                create: true,
