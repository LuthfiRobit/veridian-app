...
@section('title', 'Edit Timeline Milestone')
...
@section('content')
    <div class="page-header mb-3">
        <div class="page-block">
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title"><h5 class="m-b-10">Edit Timeline Milestone</h5></div>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                        <li class="breadcrumb-item">Company</li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.company-timelines.index') }}">Timeline</a></li>
    ...
    <form action="{{ route('admin.company-timelines.update', $timeline->id_company_timeline) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header"><h5>Translatable Content</h5></div>
                    <div class="card-body">
                        ...
                        <div class="tab-content">
                            @foreach($sortedLanguages as $language)
                                @php $t = $timeline->translate($language->code); @endphp
                                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-{{ $language->code }}" role="tabpanel">
                                    <div class="mb-3">
                                        <label class="form-label">Title ({{ $language->code }}) @if($loop->first)<span class="text-danger">*</span>@endif</label>
                                        <input type="text" class="form-control @error('translations.' . $language->code . '.title') is-invalid @enderror"
                                            ...
                                            value="{{ old('translations.' . $language->code . '.title', $t?->title) }}"
                                            @if($loop->first) required @endif>
                                        @error('translations.' . $language->code . '.title')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Description ({{ $language->code }}) @if($loop->first)<span class="text-danger">*</span>@endif</label>
                                        <textarea class="form-control @error('translations.' . $language->code . '.description') is-invalid @enderror"
                                            ...
                                            rows="4">{{ old('translations.' . $language->code . '.description', $t?->description) }}</textarea>
                                        @error('translations.' . $language->code . '.description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="col-lg-4">
                <div class="card">
                    <div class="card-header"><h5>Settings & Publish</h5></div>
                    <div class="card-body">
                        <div class="mb-3">
                            <label class="form-label">Year <span class="text-danger">*</span></label>
                            <input type="text" class="form-control @error('year') is-invalid @enderror"
                                name="year" value="{{ old('year', $timeline->year) }}" required>
                            @error('year')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Icon <span class="text-danger">*</span></label>
                            <select id="icon_class" name="icon_class" class="form-control" placeholder="Select Icon"></select>
                            <small class="text-muted">Type to search Bootstrap Icons</small>
                            @error('icon_class')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Sort Order</label>
                            <input type="number" class="form-control" name="sort_order" value="{{ old('sort_order', $timeline->sort_order) }}">
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Status</label>
                            <select class="form-select" name="is_active">
                                <option value="1" {{ old('is_active', $timeline->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $timeline->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>

                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-primary">Update Milestone</button>
                            <a href="{{ route('admin.company-timelines.index') }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const bootstrapIcons = [
                ...
                "bi-rocket", "bi-rocket-takeoff", "bi-trophy-fill", "bi-mortarboard",
                "bi-graph-up-arrow", "bi-bar-chart", "bi-clipboard-check", "bi-bullseye"
            ];
            const iconOptions = bootstrapIcons.map(icon => ({ value: 'bi ' + icon, text: 'bi ' + icon }));
            new TomSelect("#icon_class", {
                maxOptions: null,
                options: iconOptions,
                items: {!! json_encode([old('icon_class', $timeline->icon_class)]) !!},
                create: true,
